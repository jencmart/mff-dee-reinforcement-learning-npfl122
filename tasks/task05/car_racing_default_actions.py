...
def choose_action(q_value, generator, epsilon, args):
    ...
    acc = args.cnt_steer
    if 0 <= action_idx < acc:
        return [np.linspace(-0.9, 0.9, args.cnt_steer)[action_idx], 0, 0], action_idx

    acc += args.cnt_gas
    if args.cnt_steer <= action_idx < acc:
        return [0, np.linspace(0.1, 0.9, args.cnt_gas)[action_idx - args.cnt_steer], 0], action_idx

    acc += args.cnt_brake
    if args.cnt_gas <= action_idx < acc:
        return [0, 0, np.linspace(0.1, 0.9, args.cnt_brake)[action_idx - args.cnt_steer - args.cnt_gas]], action_idx
...
def main(env, args):
    ...
    if args.recodex:
        ...
        # weights = collections.deque(maxlen=20000)  # 500*20
        Transition = collections.namedtuple("Transition", ["state", "action", "reward", "done", "next_state"])
        ...
        while True:
            ...
            # plot_data(means_returns)

            # Save weights ...
            if len(returns) > 15:
                if np.average(returns[-15:]) > best_so_far:
                    best_so_far = np.average(returns[-15:])
